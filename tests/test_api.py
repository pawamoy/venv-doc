...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `venv_doc`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in venv_doc.__all__ or not hasattr(venv_doc, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in venv_doc.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("venv_doc")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "venv_doc" or item.name.startswith("venv_doc."))
        ):
